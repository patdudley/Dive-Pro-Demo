...
const laJollaShore = swellArrowWorldGeometry({
  ...defaultSwellArrowSpec("primary"),
  sourceBearing: 193,
  travelBearing: 90,
});
assert(laJollaShore.travelBearing === 90, `La Jolla arrow travel ${laJollaShore.travelBearing}, expected due east`);
assert(laJollaShore.rotateDeg === 0, `La Jolla arrow rotate ${laJollaShore.rotateDeg}, expected right/east`);

const anacapaShore = swellArrowWorldGeometry({
  ...defaultSwellArrowSpec("primary"),
  sourceBearing: 45,
  travelBearing: 45,
});
assert(anacapaShore.travelBearing === 45, `Anacapa arrow travel ${anacapaShore.travelBearing}, expected northeast`);

if (failed) {
  console.error(`FAIL ${failed} assertion(s)`);
  process.exit(1);
}
console.log(`ok ${travelCases.length} travel + 2 shore-facing + ${rotateCases.length} rotate + separation`);
