...
} from "./swell-bearing.js?v=pages-20260831shorefix3";
...
function renderSwellCompassRose(rows) {
  ...
  const primarySpec = primary
    ? { ...defaultSwellArrowSpec("primary"), sourceBearing: Number(primary.directionDeg), spot }
    : null;
  const secondarySpec = secondary
    ? { ...defaultSwellArrowSpec("secondary"), sourceBearing: Number(secondary.directionDeg), spot }
    : null;
    ...
    window.__diveProSwellCardDebug = {
      ...
      rows: rows.map((row) => ({
        ...
        arrowTravel: Number.isFinite(row.directionDeg)
          ? swellTravelBearingForSpot(row.directionDeg, currentSpot())
          : null,
        arrowRotate: Number.isFinite(row.directionDeg)
          ? swellTravelBearingToArrowRotateDeg(swellTravelBearingForSpot(row.directionDeg, currentSpot()))
          : null,
